    ...
    with open(out_path, 'a', newline='') as file:
        ...
        if question_mode == 0:
            ...
            for node in tqdm(sampled_nodes):
                if cur_level == 1:
                    cur_template = ('shopping-ebay', child2parent_dict_1[node][0], node, cur_level, 'level-positive')
                else:
                    cur_template = ('shopping-ebay', child2parent_dict_2[node][0], node, cur_level, 'level-positive')
            ...
            for node in tqdm(sampled_nodes):
                cur_template = ('shopping-ebay', random.choice(nodes_uncles_dict[node]), node, cur_level, 'level-negative-hard')
                csv_writer.writerow(cur_template)
            ...
            for node in tqdm(sampled_nodes):
                if cur_level == 1:
                    cur_template = ('shopping-ebay', random.choice(list(set(level_nodes[cur_level-1])-set(child2parent_dict_1[node]))), node, cur_level, 'level-negative-easy')
                else:
                    cur_template = ('shopping-ebay', random.choice(list(set(level_nodes[cur_level-1])-set(child2parent_dict_2[node]))), node, cur_level, 'level-negative-easy')
                csv_writer.writerow(cur_template)
            print('size of the negative set', len(sampled_nodes), cur_level)
...
for cur_question_mode in range(3):
    out_path = 'TaxoGlimpse/question_pools/shopping-ebay/level/question_pool_full_' + file_name[cur_question_mode]
    with open(out_path, 'a', newline='') as file:
        csv_writer = csv.writer(file)
