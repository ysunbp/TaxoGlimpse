...
def process_trees(all_trees, level): # level这里从0开始数
    exclusion = ['Books', 'Collectibles and Fine Arts', 'Handmade', 'Magazine Subscriptions', 'Apps and Games', 'Movies and TV', 'CDs and Vinyl', 'Digital Music', 'Amazon Instant Video', 'Clothing, Shoes and Jewelry']
    partial_exclusion = ['Baby', 'Clothing, Shoes and Jewelry - Baby', 'Clothing, Shoes and Jewelry - Boys', 'Clothing, Shoes and Jewelry - Girls', 'Clothing, Shoes and Jewelry - Men', 'Clothing, Shoes and Jewelry - Women']
    ...
    if level < 3:
        for cur_tree in all_trees:
            if cur_tree.name in exclusion:
                continue
            if (cur_tree.name in partial_exclusion) and level == 0:
                continue
            cur_level_root_nodes = get_nodes_at_level(cur_tree, level)
            for cur_root_node in cur_level_root_nodes:
                cur_sub_nodes = cur_root_node.children
                for cur_sub_node in cur_sub_nodes:
                    cur_pair = (cur_root_node.name, cur_sub_node.name)
        ...
        for node_idx in range(level, 9):
            for cur_tree in all_trees:
                if cur_tree.name in exclusion:
                    continue
                cur_level_root_nodes = get_nodes_at_level(cur_tree, node_idx)
                for cur_root_node in cur_level_root_nodes:
                    cur_sub_nodes = cur_root_node.children
                    for cur_sub_node in cur_sub_nodes:
                        cur_pair = (cur_root_node.name, cur_sub_node.name, node_idx)
                        if not str(cur_pair) in cur_level_nodes_str:
                            cur_level_nodes_str.append(str(cur_pair))
                            cur_level_nodes.append(cur_pair)
...
def process_trees_specific(all_trees, level): # level这里从0开始数
    exclusion = ['Books', 'Collectibles and Fine Arts', 'Handmade', 'Magazine Subscriptions', 'Apps and Games', 'Movies and TV', 'CDs and Vinyl', 'Digital Music', 'Amazon Instant Video', 'Clothing, Shoes and Jewelry']
    partial_exclusion = ['Baby', 'Clothing, Shoes and Jewelry - Baby', 'Clothing, Shoes and Jewelry - Boys', 'Clothing, Shoes and Jewelry - Girls', 'Clothing, Shoes and Jewelry - Men', 'Clothing, Shoes and Jewelry - Women']
        ...
        for cur_tree in all_trees:
            ...
            for cur_root_node in cur_level_root_nodes:
                ...
                for cur_sub_node in cur_sub_nodes:
                    if (cur_tree.name in exclusion):
                        if level > 0:
                            cur_pair = (cur_root_node.name + ' (' + cur_tree.name + ')' , cur_sub_node.name + ' (' + cur_tree.name + ')' )
                        else:
                            cur_pair = (cur_root_node.name, cur_sub_node.name + ' (' + cur_tree.name + ')' )
        ...
        for node_idx in range(level, 9):
            for cur_tree in all_trees:
                ...
                for cur_root_node in cur_level_root_nodes:
                    ...
                    for cur_sub_node in cur_sub_nodes:
                        if (cur_tree.name in exclusion):
                            cur_pair = (cur_root_node.name + ' (' + cur_tree.name + ')' , cur_sub_node.name + ' (' + cur_tree.name + ')' , node_idx)
                        else:
                            cur_pair = (cur_root_node.name, cur_sub_node.name, node_idx)
                        if not str(cur_pair) in cur_level_nodes_str:
                            cur_level_nodes_str.append(str(cur_pair))
...
def process_trees_hard(all_trees, level): # level这里从0开始数
...
def process_trees_root(all_trees, level): # level这里从0开始数
    exclusion = ['Books', 'Collectibles and Fine Arts', 'Handmade', 'Magazine Subscriptions', 'Apps and Games', 'Movies and TV', 'CDs and Vinyl', 'Digital Music', 'Amazon Instant Video', 'Clothing, Shoes and Jewelry', 'Kindle Store']
    partial_exclusion = ['Baby', 'Clothing, Shoes and Jewelry - Baby', 'Clothing, Shoes and Jewelry - Boys', 'Clothing, Shoes and Jewelry - Girls', 'Clothing, Shoes and Jewelry - Men', 'Clothing, Shoes and Jewelry - Women']
            ...
            for cur_root_node in cur_level_root_nodes:
                ...
                for cur_sub_node in cur_sub_nodes:
                    if (cur_tree.name in exclusion):
                        ...
                        cur_pair = (cur_tree.name, cur_sub_node.name + ' (' + cur_tree.name + ')' )
                    elif ((cur_tree.name in partial_exclusion) and level == 0):
                        cur_pair = (cur_tree.name, cur_sub_node.name + ' (' + cur_tree.name + ')' )
                    else:
                        cur_pair = (cur_tree.name, cur_sub_node.name)
                    if not str(cur_pair) in cur_level_nodes_str:
                        cur_level_nodes_str.append(str(cur_pair))
                        cur_level_nodes.append(cur_pair)
                        if level == 0:
        ...
        for node_idx in range(level, 9):
            for cur_tree in all_trees:
                ...
                for cur_root_node in cur_level_root_nodes:
                    ...
                    for cur_sub_node in cur_sub_nodes:
                        if (cur_tree.name in exclusion):
                            cur_pair = (cur_tree.name , cur_sub_node.name + ' (' + cur_tree.name + ')' , node_idx)
                        else:
                            cur_pair = (cur_tree.name, cur_sub_node.name, node_idx)
                        if not str(cur_pair) in cur_level_nodes_str:
                            cur_level_nodes_str.append(str(cur_pair))
                            cur_level_nodes.append(cur_pair)
                            cur_level_nodes_uncles.append(cur_root_node[1])
...
for cur_question_mode in range(3):
    out_path = 'TaxoGlimpse/question_pools/shopping-amazon/level/level_question_pool_full_' + file_name[cur_question_mode]
    with open(out_path, 'a', newline='') as file:
        csv_writer = csv.writer(file)
        csv_writer.writerow(['domain', 'parent', 'child', 'level', 'type'])  # 写入表头
    for cur_level in range(len(num_question_samples)):
        if cur_question_mode == 1 or cur_question_mode == 0:
            cur_level_pairs, cur_level_pairs_uncles = process_trees_hard(all_trees, level=cur_level)
        else:
            cur_level_pairs = process_trees_specific(all_trees, level=cur_level)
            cur_level_pairs_uncles = 0
        sample_nodes_hard(cur_level_pairs, cur_level_pairs_uncles, sample_size=num_question_samples[cur_level], out_path=out_path, cur_level=cur_level, question_mode=cur_question_mode)

for cur_question_mode in range(3, 5):
    out_path = 'TaxoGlimpse/question_pools/shopping-amazon/toroot/question_pool_full_' + file_name[cur_question_mode]
    with open(out_path, 'a', newline='') as file:
        csv_writer = csv.writer(file)
        csv_writer.writerow(['domain', 'parent', 'child', 'level', 'type'])  # 写入表头
    for cur_level in range(len(num_question_samples)):
        cur_level_pairs, cur_level_pairs_uncles = process_trees_root(all_trees, level=cur_level)
        sample_nodes_hard(cur_level_pairs, cur_level_pairs_uncles, sample_size=num_question_samples[cur_level], out_path=out_path, cur_level=cur_level, question_mode=cur_question_mode)
