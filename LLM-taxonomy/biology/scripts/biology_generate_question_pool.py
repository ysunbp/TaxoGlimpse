    ...
    with open(out_path, 'a', newline='') as file:
        ...
        if question_mode == 1: #negative
            ...
            for (root, child) in tqdm(negative_sampled_pairs):
                ...
                cur_template = ('biology-ncbi', root_name, child_name, cur_level, 'level-negative-hard')
            ...
            for (root, child) in tqdm(sampled_pairs):
                root_name = name_dict[root]
                child_name = name_dict[child]
                cur_template = ('biology-ncbi', root_name, child_name, cur_level, 'level-positive')
                csv_writer.writerow(cur_template)
            ...
            for (root, child) in tqdm(negative_sampled_pairs):
                root_name = name_dict[root]
                child_name = name_dict[child]
                cur_template = ('biology-ncbi', root_name, child_name, cur_level, 'level-negative-easy')
                csv_writer.writerow(cur_template)
            ...
            if len(cur_level_pairs) < sample_size:
                ...
                sampled_pairs = random.sample(cur_level_pairs, sample_size)
            for (root, child) in tqdm(sampled_pairs):
                root_name = name_dict[get_node_root(node_map_to_parent, (root, child), base_set)]
                child_name = name_dict[child]
                cur_template = ('biology-ncbi', root_name, child_name, cur_level, 'toroot-positive')
                csv_writer.writerow(cur_template)
            print('size of the positive set', len(sampled_pairs), cur_level)
            ...
            root_set = []
            for pair in tqdm(cur_level_pairs):
                if get_node_root(node_map_to_parent, pair, base_set) in root_set:
                    continue
                else:
                    root_set.append(get_node_root(node_map_to_parent, pair, base_set))
            if len(cur_level_pairs) < sample_size:
                sampled_pairs = cur_level_pairs
            else:
                sampled_pairs = random.sample(cur_level_pairs, sample_size)
            ...
            for sampled_pair in sampled_pairs:
                cur_root_set = list(set(root_set)-set([get_node_root(node_map_to_parent, sampled_pair, base_set)]))
                negative_sampled_pairs.append((random.choice(cur_root_set),sampled_pair[1]))
            for (root, child) in tqdm(negative_sampled_pairs):
                root_name = name_dict[root]
                child_name = name_dict[child]
                cur_template = ('biology-ncbi', root_name, child_name, cur_level, 'toroot-negative')
                csv_writer.writerow(cur_template)
...
name_dict_path = '/export/data/LLM-benchmark-project-KB/LLM-taxonomy/biology/NBCI/taxdmp/names.dmp'
node_path = '/export/data/LLM-benchmark-project-KB/LLM-taxonomy/biology/NBCI/taxdmp/nodes.dmp'

for idx in range(6):
    cur_level_pairs = process_node_file(node_path, idx+1, levels)
    with open('/export/data/LLM-benchmark-project-KB/LLM-taxonomy/biology/temp/level_'+str(idx+1)+'.pkl', 'wb') as pickle_file:
        pickle.dump(cur_level_pairs, pickle_file)
    print(len(cur_level_pairs))
    #print(name_dict['4751'], name_dict['33090'], name_dict['33154'], name_dict['2759'])
########################################################
########################################################
########################################################
...
for idx in range(6):
    ...
    for cur_question_mode in range(3):
        out_path = 'TaxoGlimpse/question_pools/biology-NCBI/level/level_question_pool_full_level_' + str(cur_level) + '_' + file_name[cur_question_mode]
        with open(out_path, 'a', newline='') as file:
            csv_writer = csv.writer(file)
    ...
    base_set = get_base_root_set()
    del node_map_to_parent['1']
    updated_level_pairs = []
    for level_pair in tqdm(cur_level_pairs):
        if get_node_root(node_map_to_parent, level_pair, base_set):
            updated_level_pairs.append(level_pair)

    for cur_question_mode in range(3,5):
        out_path = 'TaxoGlimpse/question_pools/biology-NCBI/toroot/question_pool_full_level_' + str(cur_level) + '_' + file_name[cur_question_mode]
        with open(out_path, 'a', newline='') as file:
            csv_writer = csv.writer(file)
            csv_writer.writerow(['domain', 'parent', 'child', 'level', 'type'])  # 写入表头
        sample_nodes_hard(updated_level_pairs, sample_size=num_question_samples[idx], out_path=out_path, cur_level=cur_level, name_dict=name_dict, node_map_to_parent = node_map_to_parent, parent_map_to_nodes=parent_map_to_nodes, question_mode=cur_question_mode, base_set=base_set)
