...
def sample_question_pool(nodes_level_list, language_dict, cur_level, sample_size, out_path, question_mode = 0):
    with open(out_path, 'a', newline='') as file:
        ...
        if question_mode == 0: #MCQ negative hard
            root_set = []
            ...
            num_pairs = 0
            for idx, node in enumerate(tqdm(sampled_nodes)):
                cur_subtype = get_name(node, language_dict)
                cur_root_base = get_parent(node, language_dict)

                if not cur_level == 1: # hard start
                    cur_root_base = get_parent_node(node)
                    cur_grand = get_parent_node(cur_root_base) 
                    #print(cur_level, get_name(node, language_dict), get_name(cur_root_base, language_dict), get_name(cur_grand, language_dict), [get_name(item, language_dict) for item in cur_grand.children])
                    hard_root_set = cur_grand.children
                    if not len(hard_root_set) < 4: # include its own parents
                        root_set = []
                        ...
                        #print([get_name(cur_root_base, language_dict)], root_set)
                        selected_mcqs = random.sample(cur_root_set, 3)
                    else:
                        hard_root_set_names = list(set([get_name(item, language_dict) for item in hard_root_set])-set([get_name(cur_root_base, language_dict)]))
                        sampled_indices = random.sample(list(set(list(range(len(sampled_nodes))))-set([idx])), 4-len(hard_root_set))
                        sampled_complements = [get_name(sampled_nodes[sampled_idx], language_dict) for sampled_idx in sampled_indices]


                        #print(hard_root_set_names)
                        #hard_root_set_names = [get_name(hard_root, language_dict) for hard_root in hard_root_set]
                        selected_mcqs = hard_root_set_names + sampled_complements
                        #print(hard_root_set_names, selected_mcqs, [get_name(cur_root_base, language_dict)])
                else:
                ...
                num_pairs += 1
                cur_root = get_parent(node, language_dict)
                cur_template = ('language-glottolog', cur_root, cur_subtype, cur_level, 'mcq-negative-hard', selected_mcqs[0], selected_mcqs[1], selected_mcqs[2])
                csv_writer.writerow(cur_template)                
            print('size of the MCQ set', num_pairs, cur_level)
        
        elif question_mode == 1: #MCQ negative easy
            root_set = []
            for node in nodes_level_list:
                cur_root = get_parent(node, language_dict)
                if cur_root in root_set:
            ...
            for idx, node in enumerate(tqdm(sampled_nodes)):
                ...
                cur_root_base = get_parent(node, language_dict)
                cur_root_set = list(set(root_set)-set([cur_root_base]))
                #print(cur_root_set, cur_root_base)
                if len(cur_root_set) < 3:
                    sampled_indices = random.sample(list(set(list(range(len(sampled_nodes))))-set([idx])), 3-len(cur_root_set))
                    sampled_complements = [get_name(sampled_nodes[sampled_idx], language_dict) for sampled_idx in sampled_indices]
                    selected_mcqs = cur_root_set + sampled_complements
                else:
                    selected_mcqs = random.sample(cur_root_set, 3)
                num_pairs += 1
                cur_template = ('language-glottolog', cur_root, cur_subtype, cur_level, 'mcq-negative-easy', selected_mcqs[0], selected_mcqs[1], selected_mcqs[2])
                csv_writer.writerow(cur_template)                
            print('size of the MCQ set', num_pairs, cur_level)
...
for cur_question_mode in range(2):
    out_path = 'TaxoGlimpse/question_pools/language-glottolog/mcq/question_pool_full_' + file_name[cur_question_mode]
    with open(out_path, 'a', newline='') as file:
        csv_writer = csv.writer(file)
