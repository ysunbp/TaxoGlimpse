        ...
        for cur_key in cur_question_pool_dict.keys():
            ...
            if exp_type == 'few-shot':
                if sample_type == 'positive':
                    ...
                    #cur_level_bank_negative = question_bank['level_'+cur_key_level+'_negative_easy'] # Modified due to experimental requirements.
            ...
            with open(cur_path, 'a', newline='') as file:
                ...
                for idx, (roottype_o, subtype_o) in enumerate(tqdm(cur_sampled_question_pairs[sampled_question_key])):
                    ...
                    for roottype, subtype in cur_positive_examples:
                        example = 'Example: '+compose_question_templates(taxonomy_type, subtype, roottype, variant_id)
                        example_answer = 'Yes.'
                        dialog['message'].append((example,example_answer))
...
if __name__ == "__main__":
    ...
    cur_model_dir = 'Meta-Llama-3-8B-Instruct/'
        ...
        if exp_type == 'zero-shot' or exp_type == 'COT':  
            for variant_id in variant_ids:
                for question_pool_name in question_pool_names:      
                    ...
                    #sampled_question_pairs = get_sampled_pairs(sub_question_type='level', level_question_types=['negative_hard'], toroot_question_types=None, question_pool_name = question_pool_name)
            ...
            for variant_id in variant_ids:
                for question_pool_name in question_pool_names:        
                    ...
                    for cur_question_pair, cur_few_shot in cur_question_pairs:
                        compose_questions_get_response_ctd((cur_question_pair, cur_few_shot), taxonomy_type=question_pool_name, generator=generator, ckpt_dir = cur_model_dir, exp_type=exp_type, variant_id=variant_id) # GPU29
