...
def compute_cur_level_size(total_samples):
    ...
    #return 360 # for google instance typing
...
def get_question_pool(sampled_question_pairs, taxonomy_type):
    ...
    clue = 'Always answer with Yes, No, or I don\'t know. '
    ...
    for sampled_question_key in sampled_question_pairs.keys():
        ...
        for (roottype, subtype) in tqdm(sampled_question_pairs[sampled_question_key]):
            if taxonomy_type == 'academic-acm':
                dialog = clue + 'Is ' + subtype + ' computer science research concept a type of '+roottype +' computer science research concept? answer with (Yes/No/I don\'t know)'
            elif taxonomy_type == 'biology-NCBI':
                dialog = clue + 'Is ' + subtype + ' a type of '+roottype +'? answer with (Yes/No/I don\'t know)'
            elif taxonomy_type == 'language-glottolog':
                dialog = clue + 'Is ' + subtype + ' language a type of '+roottype +' language? answer with (Yes/No/I don\'t know)'
            elif taxonomy_type == 'medical-icd':
                dialog = clue + 'Is ' + subtype.lower() + ' a type of '+roottype.lower() +'? answer with (Yes/No/I don\'t know)'
            elif taxonomy_type == 'shopping-amazon':
                dialog = clue + 'Are ' + subtype + ' products a type of '+roottype +' products? answer with (Yes/No/I don\'t know)'
            elif taxonomy_type == 'shopping-google':
                dialog = clue + 'Are ' + subtype + ' products a type of '+roottype +' products? answer with (Yes/No/I don\'t know)'
            dialog += "Let's think step by step."
    ...
    try:
        if args.question_path:
            ...
            for cur_taxonomy in args.question_path:
                ...
                #sampled_question_pairs = get_sampled_pairs(sub_question_type='level', level_question_types=['negative_easy', 'positive'], toroot_question_types=None, question_pool_name = cur_taxonomy)
                ...
                for cur_sampled_question_pairs in cur_question_pairs:
                    ...
                    for cur_inps_key in inps_dict.keys():
                        ...
                        cur_path = 'TaxoGlimpse/results/instance-full-COT/'+cur_taxonomy+'/'+args.model_path.split('/')[1]+'/'+cur_question_cat+'.csv'
                        yes_total = 0
                        no_total = 0
                        dont_total = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question_path", type=list, default=['biology-NCBI', 'language-glottolog', 'medical-icd', 'shopping-amazon', 'shopping-google'], choices=['academic-acm', 'biology-NCBI', 'language-glottolog', 'medical-icd', 'shopping-amazon', 'shopping-google'])
    ...
    main(args)
    ### command: python3 -m fastchat.serve.cli-COT-instance-full --model-path lmsys/vicuna-7b-v1.5 >> /export/data/LLM-benchmark-project-KB/logs/vicuna-7b/instance-amazon-full-COT.txt
    ### command: python3 -m fastchat.serve.cli-COT-instance-full --model-path lmsys/vicuna-13b-v1.5 >> /export/data/LLM-benchmark-project-KB/logs/vicuna-13b/instance-amazon-full-COT.txt
    ### command: python3 -m fastchat.serve.cli-COT-instance-full --model-path lmsys/vicuna-33b-v1.3 >> /export/data/LLM-benchmark-project-KB/logs/vicuna-33b/instance-amazon-full-COT.txt