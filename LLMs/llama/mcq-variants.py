...
def compose_questions_get_response_ctd(sampled_question_pairs, taxonomy_type, generator, ckpt_dir = 'llama-2-7b-chat/', exp_type='zero-shot', variant_id=0): # MODIFIED
    answer_notes = ['A)', 'B)', 'C)', 'D)']
    if exp_type == 'mcq':
        ...
        for sampled_question_key in sampled_question_pairs.keys():
            ...
            with open(cur_path, 'a', newline='') as file:
                ...
                for (roottype, subtype, alternatives) in tqdm(sampled_question_pairs[sampled_question_key]):
                    ...
                    ####
                    if correct_ground_truth in llama_current_answer:
                        decision = 1
                        yes_total += 1
                        ...
                        decision = 0
                        no_total += 1
                    cur_row = (roottype, subtype, dialog['user'], correct_ground_truth, llama_current_answer, decision)
                    if dist.get_rank() == 0:
...
if __name__ == "__main__":
    ...
    cur_model_dir = 'llama-2-70b-chat/'
    ...
    ### current file path: TaxoGlimpse/LLMs/llama/mcq-variants.py 
    ### command: torchrun --nproc_per_node 1 mcq-variants.py 
    ### command: torchrun --nproc_per_node 2 mcq-variants.py 
    ### command: torchrun --nproc_per_node 8 mcq-variants.py 
