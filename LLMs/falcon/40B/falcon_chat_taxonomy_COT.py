        ...
        with open(cur_path, 'a', newline='') as file:
            ...
            for (roottype, subtype) in tqdm(sampled_question_pairs[sampled_question_key]):
                if taxonomy_type == 'academic-acm':
                    dialog = system_message+'Is ' + subtype + ' computer science research concept a type of '+roottype +' computer science research concept? ' + cot_message+ '\nAnswer:'
                elif taxonomy_type == 'biology-NCBI':
                    dialog = system_message+'Is ' + subtype + ' a type of '+roottype +'? ' + cot_message+ '\nAnswer:'
                elif taxonomy_type == 'language-glottolog':
                    dialog = system_message+'Is ' + subtype + ' language a type of '+roottype +' language? ' + cot_message+ '\nAnswer:'
                elif taxonomy_type == 'medical-icd':
                    dialog = system_message+'Is ' + subtype.lower() + ' a type of '+roottype.lower() +'? ' + cot_message+ '\nAnswer:'
                elif taxonomy_type == 'shopping-amazon':
                    dialog = system_message+'Are ' + subtype + ' products a type of '+roottype +' products? ' + cot_message+ '\nAnswer:'
                ...
                falcon_decision = falcon_answer[:15]

                if 'yes' in falcon_decision:
                    decision = 1
                    yes_total += 1
                elif 'know' in falcon_decision:
                    decision = 2
                    dont_total += 1
                else:
                    decision = 0
