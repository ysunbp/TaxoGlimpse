...
def compose_questions_get_response(sampled_question_pairs, taxonomy_type, pipeline, tokenizer, model_name='flan-t5-3b'):
    ...
    for sampled_question_key in sampled_question_pairs.keys():
        ...
        cur_path = 'TaxoGlimpse/results/instance-full-zero/'+taxonomy_type+'/'+model_name+'/'+cur_question_cat+'.csv'
        yes_total = 0
        no_total = 0
        dont_total = 0
        with open(cur_path, 'a', newline='') as file:
            ...
            for (roottype, subtype) in tqdm(sampled_question_pairs[sampled_question_key]):
                if taxonomy_type == 'academic-acm':
                    dialog = system_message+'Is ' + subtype + ' computer science research concept a type of '+roottype +' computer science research concept? answer with (Yes/No/I don\'t know)' + '\nAnswer:'
                elif taxonomy_type == 'biology-NCBI':
                    dialog = system_message+'Is ' + subtype + ' a type of '+roottype +'? answer with (Yes/No/I don\'t know)' + '\nAnswer:'
                elif taxonomy_type == 'language-glottolog':
                    dialog = system_message+'Is ' + subtype + ' language a type of '+roottype +' language? answer with (Yes/No/I don\'t know)' + '\nAnswer:'
                elif taxonomy_type == 'medical-icd':
                    dialog = system_message+'Is ' + subtype.lower() + ' a type of '+roottype.lower() +'? answer with (Yes/No/I don\'t know)' + '\nAnswer:'
                elif taxonomy_type == 'shopping-amazon':
                    dialog = system_message+'Are ' + subtype + ' products a type of '+roottype +' products? answer with (Yes/No/I don\'t know)' + '\nAnswer:'
                ...
                if 'yes' in falcon_answer:
                    ...
                    decision = 0
                    no_total += 1
                cur_row = (roottype, subtype, falcon_answer, decision)
...
if __name__ == "__main__":
    ...
    question_pool_names = ['biology-NCBI', 'language-glottolog', 'medical-icd', 'shopping-amazon', 'shopping-google']
    ...
    for question_pool_name in question_pool_names:
        ...
        for cur_question_pair in cur_question_pairs:
            compose_questions_get_response(cur_question_pair, taxonomy_type=question_pool_name, pipeline=pipeline, tokenizer=tokenizer, model_name='falcon-40b')
