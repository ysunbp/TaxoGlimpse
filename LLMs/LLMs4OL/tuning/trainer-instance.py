...
def sample_question_pairs(question_pool_dict):
    ...
    if 'positive' in first_question_pool_key:
        key = 'positive'
        if 'toroot' in first_question_pool_key:
            key = 'positive-toroot'
        elif 'partial' in first_question_pool_key:
            key = 'positive-partial'
    elif 'negative' in first_question_pool_key:
        key = 'negative'
        if 'toroot' in first_question_pool_key:
            key = 'negative-toroot'
        elif 'partial' in first_question_pool_key:
            key = 'negative-partial'
    setup_seed(20)        
...
def load_csv_file_amazon_toroot(csv_path, question_type):
    question_pools = {}
...
def load_csv_file_biology_toroot(csv_path, question_type):
    question_pools = {}
    with open(csv_path, 'r') as csvfile:
        csvreader = csv.reader(csvfile)
        for idx, row in enumerate(csvreader):
...
def load_csv_file_biology_partial(csv_path, question_type):
    question_pools = {}
    with open(csv_path, 'r') as csvfile:
        csvreader = csv.reader(csvfile)
        for idx, row in enumerate(csvreader):
            if idx == 0:
                ...
                cur_level = 'partial_'+row[3]+'_'+question_type
                cur_parent = row[1]
                cur_child = row[2]
                if not cur_level in question_pools.keys():
                    question_pools[cur_level] = [(cur_parent, cur_child)]
    ...
    if not question_pool_name == 'biology-NCBI':
        if sub_question_type == 'level':
            ...
            for level_question_type in level_question_types:
                ...
                sampled_question_pairs.update(sample_question_pairs(cur_question_pool_dict))
        elif sub_question_type == 'instance-zero-revision':
            if question_pool_name == 'medical-icd':
                sampled_question_pairs = {}
                for level_question_type in level_question_types:
                ...
                for level_question_type in level_question_types:
                    ...
                    if level_question_type == 'positive':
                        ...
                        sampled_question_pairs.update(sample_question_pairs(cur_question_pool_dict))
                        cur_csv_file = to_root_question_pool_path + 'question_pool_full_' + level_question_type + '_to_root.csv'
                        cur_question_pool_dict = load_csv_file_amazon_toroot(cur_csv_file, level_question_type)
                        sampled_question_pairs.update(sample_question_pairs(cur_question_pool_dict))
                    elif level_question_type == 'negative_hard':
                        cur_csv_file = cur_question_pool_path + 'question_pool_full_updated_' + level_question_type + '.csv'
                        cur_question_pool_dict = load_csv_file(cur_csv_file, level_question_type)
        ...
        if sub_question_type == 'level':
            ...
            for level_question_type in level_question_types:
                if level_question_type == 'positive':
                    ...
                    cur_question_pool_dict = load_csv_file_biology_partial(cur_csv_file, level_question_type)
                    sampled_question_pairs.update(sample_question_pairs(cur_question_pool_dict))
                    cur_csv_file = to_root_question_pool_path + 'question_pool_full_level_6_' + level_question_type + '_to_root.csv'
                    cur_question_pool_dict = load_csv_file_biology_toroot(cur_csv_file, level_question_type)
                    sampled_question_pairs.update(sample_question_pairs(cur_question_pool_dict))
                elif level_question_type == 'negative_hard':
...
def load_training_data(kb_name):
    ...
    sampled_question_pairs_tf = get_sampled_pairs(sub_question_type='instance-zero-revision', level_question_types=['positive', 'negative_hard'], toroot_question_types=None, question_pool_name = question_pool_name)
...
if __name__=="__main__":
    question_pool_names = ['medical-oae', 'biology-NCBI', 'language-glottolog', 'medical-icd', 'shopping-amazon', 'shopping-google']
    ...
    for question_pool_name in question_pool_names:
        ...
        parser.add_argument("--kb_name", default=question_pool_name+'-instance')
