...
def compose_questions_get_response(sampled_question_pairs, taxonomy_type, model, tokenizer, model_name='flan-t5-3b', variant_id=0):
    ...
    for sampled_question_key in sampled_question_pairs.keys():
        ...
        if variant_id == 0:
            folder_path = 'TaxoGlimpse/results/mcq/'+taxonomy_type+'/'+model_name+'/'
            ...
            folder_path = 'TaxoGlimpse/results/mcq-'+str(variant_id)+'/'+taxonomy_type+'/'+model_name+'/'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
        ...
        with open(cur_path, 'a', newline='') as file:
            ...
            for (roottype, subtype, alternatives) in tqdm(sampled_question_pairs[sampled_question_key]):
                ...
                if correct_ground_truth in flan_current_answer or correct_ground_truth[0] == flan_current_answer[0]:
                    decision = 1
                    yes_total += 1
                    ...
                    decision = 0
                    no_total += 1
                cur_row = (roottype, subtype, dialog, correct_ground_truth, flan_current_answer, decision)
...
if __name__ == "__main__":
    ...
    for question_pool_name in question_pool_names:
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained("TaxoGlimpse/LLMs/LLMs4OL/assets/Tuning/"+question_pool_name+"-flan-t5-xl").cuda() # 3B
        tokenizer = AutoTokenizer.from_pretrained("TaxoGlimpse/LLMs/LLMs4OL/assets/Tuning/"+question_pool_name+"-flan-t5-xl")
    
        variant_ids = [0,1,2]
        
        for variant_id in variant_ids:
            cur_question_pairs = []
            sampled_question_pairs = get_sampled_pairs(sub_question_type='mcq', level_question_types=['hard','easy'], toroot_question_types=None, question_pool_name = question_pool_name)
