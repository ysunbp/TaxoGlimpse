    ...
    with open(csv_path, 'r') as csvfile:
        ...
        for idx, row in enumerate(csvreader):
            ...
            if idx == 0:
                ...
                cur_level = 'toroot_'+row[3]+'_'+question_type
                cur_parent = row[1]
                cur_child = row[2]
...
def compose_questions_get_response(sampled_question_pairs, taxonomy_type, model, tokenizer, model_name='flan-t5-3b', variant_id=0):
    ...
    for sampled_question_key in sampled_question_pairs.keys():
        ...
        if variant_id == 0:
            folder_path = 'TaxoGlimpse/results/instance-zero-revision/'+taxonomy_type+'/'+model_name+'/'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            cur_path = 'TaxoGlimpse/results/instance-zero-revision/'+taxonomy_type+'/'+model_name+'/'+cur_question_cat+'.csv'
        else:
            folder_path = 'TaxoGlimpse/results/instance-zero-revision-'+str(variant_id)+'/'+taxonomy_type+'/'+model_name+'/'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            cur_path = 'TaxoGlimpse/results/instance-zero-revision-'+str(variant_id)+'/'+taxonomy_type+'/'+model_name+'/'+cur_question_cat+'.csv'
        yes_total = 0
        no_total = 0
        dont_total = 0
...
if __name__ == "__main__":
    ...
    for question_pool_name in question_pool_names:
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained("TaxoGlimpse/LLMs/LLMs4OL/assets/Tuning/"+question_pool_name+"-instance-flan-t5-xl").cuda() # 3B
        tokenizer = AutoTokenizer.from_pretrained("TaxoGlimpse/LLMs/LLMs4OL/assets/Tuning/"+question_pool_name+"-instance-flan-t5-xl")
        variant_ids = [0]
        
        for variant_id in variant_ids:
            cur_question_pairs = []
            sampled_question_pairs = get_sampled_pairs(sub_question_type='level', level_question_types=['positive', 'negative_hard'], toroot_question_types=None, question_pool_name = question_pool_name)
            cur_question_pairs.append(sampled_question_pairs)
            for cur_question_pair in cur_question_pairs:
                compose_questions_get_response(cur_question_pair, taxonomy_type=question_pool_name, model=model, tokenizer=tokenizer, model_name='llms4ol', variant_id=variant_id)
        del model
    ### command: python llms4ol_taxonomy_variants_template.py >> TaxoGlimpse/logs/flan-t5-11b-level_and_toroot.txt
